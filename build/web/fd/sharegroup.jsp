...
<%!
	...
	public String drawList(int iJSPCommand,JspShareGroup frmObject, ShareGroup objEntity, Vector objectClass,  long shareGroupId)

	{
		...
		for (int i = 0; i < objectClass.size(); i++) {
			 ShareGroup shareGroup = (ShareGroup)objectClass.get(i);
			 rowx = new Vector();
			 if(shareGroupId == shareGroup.getOID())
				 index = i; 

			 if(index == i && (iJSPCommand == JSPCommand.EDIT || iJSPCommand == JSPCommand.ASK)){
				//rowx.add("<div align=\"center\">"+"<input type=\"text\" size=\"13\" name=\""+frmObject.colNames[JspShareGroup.JSP_CODE] +"\" value=\""+shareGroup.getCode()+"\" class=\"formElemen\">"+"</div>");	
				rowx.add("<div align=\"center\">"+"<input type=\"text\" size=\"32\" name=\""+frmObject.colNames[JspShareGroup.JSP_NAME] +"\" value=\""+shareGroup.getName()+"\" class=\"formElemen\">"+"</div>");
				
			 }else{
				//rowx.add(shareGroup.getCode());
				rowx.add("<a href=\"javascript:cmdEdit('"+shareGroup.getOID()+"')\">"+shareGroup.getName()+"</a>");
			 } 
		...
		if(iJSPCommand == JSPCommand.ADD || (iJSPCommand == JSPCommand.SAVE && frmObject.errorSize() > 0)){ 
				...
				//rowx.add("<div align=\"center\">"+"<input type=\"text\"  size=\"13\" name=\""+frmObject.colNames[JspShareGroup.JSP_CODE] +"\" value=\""+objEntity.getCode()+"\" class=\"formElemen\">"+"</div>");
				rowx.add("<div align=\"center\">"+"<input type=\"text\"  size=\"32\" name=\""+frmObject.colNames[JspShareGroup.JSP_NAME] +"\" value=\""+objEntity.getName()+"\" class=\"formElemen\">"+"</div>");
				System.out.println("add neh");
		}
...
long oidShareGroup = JSPRequestValue.requestLong(request, "hidden_share_group_id");
...
CmdShareGroup ctrlShareGroup = new CmdShareGroup(request);
JSPLine jspLine = new JSPLine();
Vector listShareGroup = new Vector(1,1);

/*switch statement */
iErrCode = ctrlShareGroup.action(iJSPCommand , oidShareGroup);
/* end switch*/
JspShareGroup jspShareGroup = ctrlShareGroup.getForm();

/*count list All ShareGroup*/
int vectSize = DbShareGroup.getCount(whereClause);

/*switch list ShareGroup*/
if((iJSPCommand == JSPCommand.FIRST || iJSPCommand == JSPCommand.PREV )||
  (iJSPCommand == JSPCommand.NEXT || iJSPCommand == JSPCommand.LAST)){
		start = ctrlShareGroup.actionList(iJSPCommand, start, vectSize, recordToGet);
 } 
/* end switch list*/

ShareGroup shareGroup = ctrlShareGroup.getShareGroup();
msgString =  ctrlShareGroup.getMessage();

/* get record to display */
listShareGroup = DbShareGroup.list(start,recordToGet, whereClause , orderClause);

/*handle condition if size of record to display = 0 and start > 0 	after delete*/
if (listShareGroup.size() < 1 && start > 0)
{
...
function cmdAdd(){
	...
	document.frmsharegroup.action="sharegroup.jsp";
	document.frmsharegroup.submit();
}

function cmdAsk(oidShareGroup){
	document.frmsharegroup.hidden_share_group_id.value=oidShareGroup;
...
function cmdConfirmDelete(oidShareGroup){
	document.frmsharegroup.hidden_share_group_id.value=oidShareGroup;
...
function cmdEdit(oidShareGroup){
	document.frmsharegroup.hidden_share_group_id.value=oidShareGroup;
...
function cmdCancel(oidShareGroup){
	document.frmsharegroup.hidden_share_group_id.value=oidShareGroup;
	document.frmsharegroup.command.value="<%=JSPCommand.EDIT%>";
	document.frmsharegroup.prev_command.value="<%=prevJSPCommand%>";
	document.frmsharegroup.action="sharegroup.jsp";
...
function cmdDelPict(oidShareGroup){
	document.frmimage.hidden_share_group_id.value=oidShareGroup;
	document.frmimage.command.value="<%=JSPCommand.POST%>";
	document.frmimage.action="sharegroup.jsp";
                ...
                <td width="100%" valign="top"> 
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr> 
					  ...
					  String navigator = "<font class=\"lvl1\">Master</font><font class=\"tit1\">&nbsp;&raquo;&nbsp;<span class=\"lvl2\">Share Group</span></font>";
					  %>
                    ...
                    <tr> 
                      <td><!-- #BeginEditable "content" --> 
                        <form name="frmsharegroup" method ="post" action="">
                          ...
                          <input type="hidden" name="hidden_share_group_id" value="<%=oidShareGroup%>">
							...
							%>
                                        <tr align="left" valign="top"> 
                                          <td height="22" valign="middle" colspan="3"> 
                                            <%= drawList(iJSPCommand,jspShareGroup, shareGroup,listShareGroup,oidShareGroup)%> </td>
                                        </tr>
                                        <% 
						  }catch(Exception exc){ 
					...
					%>
                                  ...
                                  <tr align="left" valign="top" > 
									...
									String scomDel = "javascript:cmdAsk('"+oidShareGroup+"')";
									String sconDelCom = "javascript:cmdConfirmDelete('"+oidShareGroup+"')";
									String scancel = "javascript:cmdEdit('"+oidShareGroup+"')";
