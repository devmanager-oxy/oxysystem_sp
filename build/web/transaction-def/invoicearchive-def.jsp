...
<%
boolean payablePriv = QrUserSession.isHavePriviledge(appSessUser.getUserOID(), AppMenu.M1_MENU_ACCPAYABLE, AppMenu.M2_MENU_ACCPAYABLE_ARCHIVES);
%>
...
<%
	...
	long oidInvoiceArchive = JSPRequestValue.requestLong(request, "hidden_invoice_id");
	...
	String invNumber = JSPRequestValue.requestString(request, "invnumber");
	...
	*/
	...
	if(vndInvNumber.length()>0)
	{
		if (whereClause != "") whereClause = whereClause + " and ";
		whereClause = whereClause + "vendor_invoice_number like '%"+vndInvNumber+"%'";
	}	
	if(invNumber.length()>0)
	{
		if (whereClause != "") whereClause = whereClause + " and ";
		whereClause = whereClause + "journal_number like '%"+invNumber+"%'";
	}
	if(vendorId!=0)
	{
		if (whereClause != "") whereClause = whereClause + " and ";
		whereClause = whereClause + "vendor_id = "+vendorId;
	}	
...
<%}%>
	...
	function cmdSearch(){
		...
		document.frmbankarchive.action="invoicearchive.jsp";
		document.frmbankarchive.submit();
	}

	function cmdEditInvoice(oidInvoice){
		document.frmbankarchive.command.value="<%=JSPCommand.EDIT%>";
		document.frmbankarchive.hidden_invoice_id.value=oidInvoice;
		document.frmbankarchive.action="invoiceprint.jsp";
		document.frmbankarchive.submit();
	}
	
	function cmdEditnonpoPayment(oidInvoice){
		document.frmbankarchive.hidden_invoice_id.value=oidInvoice;
                  ...
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr> 
                      <td class="title"><!-- #BeginEditable "title" --><span class="level1">Account 
                        Payable </span> &raquo; Invoice &raquo; <span class="level2">Archives<br>
                        </span><!-- #EndEditable --></td>
                    </tr>
                    <!--tr> 
                    ...
                    <tr> 
                      <td><!-- #BeginEditable "content" --> 
                        <form name="frmbankarchive" method ="post" action="">
							...
							<input type="hidden" name="hidden_invoice_id" value="<%=oidInvoiceArchive%>">
                                ...
                                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                          ...
                                          <td colspan="4" valign="top"> 
                                            <table width="100%" border="0" cellspacing="1" cellpadding="1">
                                              ...
                                              <tr> 
                                                ...
                                                <td width="13%" nowrap>Vendor 
                                                  Invoice Number </td>
                                                <td width="57%"> 
                                                  <input type="text" name="vndinvnumber"  value="<%= vndInvNumber %>">
                                                </td>
                                              </tr>
                                              <tr> 
                                                <td width="10%">Invoice Number</td>
                                                <td width="20%"> 
                                                  <input type="text" name="invnumber"  value="<%= invNumber %>">
                                                </td>
                                              ...
                                              <tr> 
                                                <td colspan="4" height="20"><b><font size="2">Invoice 
                                                  List</font></b></td>
                                              </tr>
                                            </table>
                                          </td>
                                  ...
                                  <tr> 
                                    <td class="boxed1"> 
                                      <table width="100%" border="0" cellspacing="1" cellpadding="1">
                                        <tr> 
                                          <td width="7%" class="tablehdr">Invoice 
                                            Number</td>
                                          <td height="26" width="11%" class="tablehdr">PO 
                                            Number </td>
                                          <td height="26" width="11%" class="tablehdr">Vendor 
                                            Invoice <br>
                                            Number</td>
                                          <td height="26" width="11%" class="tablehdr">Vendor</td>
                                          <td width="17%" class="tablehdr">Vendor 
                                            Address </td>
                                          <td width="5%" class="tablehdr">Currency</td>
                                          <td width="15%" class="tablehdr">Total</td>
                                        ...
                                        <%												
													if(listInvoice!=null && listInvoice.size()>0)
													{
														...
														{															
															Invoice bd = (Invoice)listInvoice.get(i);
															//out.println("i = "+i);
															...
															Purchase xprc = new Purchase();
															try{
																xprc = DbPurchase.fetchExc(bd.getPurchaseId());
															}
															catch(Exception e){
															}
											...
											%>
                                        <tr> 
                                          <td class="tablecell" width="7%"><a href="javascript:cmdEditInvoice('<%=bd.getOID()%>')"><%=bd.getJournalNumber()%></a></td>
                                          <td class="tablecell" width="11%"><%=xprc.getJournalNumber()%></td>
                                          <td class="tablecell" width="11%"><a href="javascript:cmdEditInvoice('<%=bd.getOID()%>')"><%=bd.getVendorInvoiceNumber()%></a></td>
                                          <td class="tablecell" width="11%"><%=vnd.getCode()+" - "+vnd.getName()%></td>
                                          <td class="tablecell" width="17%"><%=vnd.getAddress()%></td>
                                          ...
                                          <td class="tablecell" width="15%"> 
                                            <div align="right"><%=JSPFormater.formatNumber(bd.getGrandTotal(), "#,###.##")%></div>
                                          </td>
                                          ...
                                          <td class="tablecell" width="7%"> 
                                            <div align="center">
                                              <%if(bd.getActivityStatus().equals(I_Project.NA_POSTED_STATUS)){%>
                                        ...
                                        <%
											...
											%>
                                        <tr> 
                                          <td class="tablecell1" width="7%"><a href="javascript:cmdEditInvoice('<%=bd.getOID()%>')"><%=bd.getJournalNumber()%></a></td>
                                          <td class="tablecell1" width="11%"><%=xprc.getJournalNumber()%></td>
                                          <td class="tablecell1" width="11%"><a href="javascript:cmdEditInvoice('<%=bd.getOID()%>')"><%=bd.getVendorInvoiceNumber()%></a></td>
                                          <td class="tablecell1" width="11%"><%=vnd.getCode()+" - "+vnd.getName()%></td>
                                          <td class="tablecell1" width="17%"><%=vnd.getAddress()%></td>
                                        ...
                                        <tr> 
                                          <td class="tablecell1" colspan="10" height="25"> 
