...
<%!
...

	{
		...
		for (int i = 0; i < objectClass.size(); i++) {
			PeriodeRekap periode = (PeriodeRekap)objectClass.get(i);
...
long oidPeriodeRekap = JSPRequestValue.requestLong(request, "hidden_periode_id");
...
CmdPeriodeRekap ctrlPeriodeRekap = new CmdPeriodeRekap(request);
JSPLine ctrLine = new JSPLine();
Vector listPeriodeRekap = new Vector(1,1);

/*switch statement */
iErrCode = ctrlPeriodeRekap.action(iJSPCommand , oidPeriodeRekap);
/* end switch*/
JspPeriodeRekap jspPeriodeRekap = ctrlPeriodeRekap.getForm();

/*count list All PeriodeRekap*/
int vectSize = DbPeriodeRekap.getCount(whereClause);

PeriodeRekap periode = ctrlPeriodeRekap.getPeriodeRekap();
msgString =  ctrlPeriodeRekap.getMessage();


if((iJSPCommand == JSPCommand.FIRST || iJSPCommand == JSPCommand.PREV )||
  (iJSPCommand == JSPCommand.NEXT || iJSPCommand == JSPCommand.LAST)){
		start = ctrlPeriodeRekap.actionList(iJSPCommand, start, vectSize, recordToGet);
 } 
/* end switch list*/

/* get record to display */
listPeriodeRekap = DbPeriodeRekap.list(start,recordToGet, whereClause , orderClause);

/*handle condition if size of record to display = 0 and start > 0 	after delete*/
if (listPeriodeRekap.size() < 1 && start > 0)
{
...
<%
	//get open period id
	PeriodeRekap p = new PeriodeRekap();
	try{
		p = DbPeriodeRekap.getOpenPeriod();
	}catch(Exception e){
	...
	if (iJSPCommand==CMD_CLOSE){
		if((pcPayment!=null && pcPayment.size()>0) || (pcReplenishment!=null && pcReplenishment.size()>0) || (bankPO!=null && bankPO.size()>0) || (bankNonPO!=null && bankNonPO.size()>0) || (inv!=null && inv.size()>0)){
			...
			//response.sendRedirect("perioderekap.jsp?menu_idx=6&cmd="+JSPCommand.ADD);
...
function cmdAdd(){
	...
	document.frmperiode.action="perioderekap.jsp";
	document.frmperiode.submit();
}

function cmdAsk(oidPeriodeRekap){
	document.frmperiode.hidden_periode_id.value=oidPeriodeRekap;
...
function cmdConfirmDelete(oidPeriodeRekap){
	document.frmperiode.hidden_periode_id.value=oidPeriodeRekap;
...
function cmdEdit(oidPeriodeRekap){
	document.frmperiode.hidden_periode_id.value=oidPeriodeRekap;
...
function cmdCancel(oidPeriodeRekap){
	document.frmperiode.hidden_periode_id.value=oidPeriodeRekap;
	document.frmperiode.command.value="<%=JSPCommand.EDIT%>";
	document.frmperiode.prev_command.value="<%=prevJSPCommand%>";
	document.frmperiode.action="perioderekap.jsp";
                  ...
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr> 
                      <td class="title"><!-- #BeginEditable "title" --><span class="level1">Simpan 
                        Pinjam </span> &raquo; <span class="level2">Periode Rekap<br>
                        </span><!-- #EndEditable --></td>
                    </tr>
                    <!--tr> 
                    ...
                    <tr> 
                      <td><!-- #BeginEditable "content" --> 
                        <form name="frmperiode" method ="post" action="">
                          ...
                          <input type="hidden" name="hidden_periode_id" value="<%=oidPeriodeRekap%>">
                                  ...
                                  <tr align="left" valign="top"> 
                                    <td height="8"  colspan="3"> 
                                      <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                        ...
                                        <%
							try{
								if (listPeriodeRekap.size()>0){
							%>
                                        <tr align="left" valign="top"> 
                                          <td height="22" valign="middle" colspan="3"> 
                                            <table width="70%" border="0" cellspacing="0" cellpadding="0">
                                              <tr> 
                                                <td class="boxed1"><%= drawList(listPeriodeRekap,oidPeriodeRekap)%></td>
                                              </tr>
                                            </table>
                                          </td>
                                        </tr>
								...
								ctrLine.setFirstImage("<img name=\"Image23x\" border=\"0\" src=\""+approot+"/images/first.gif\" alt=\"First\">");
								 ...
								 %>
                                        ...
                                        <%if(iJSPCommand!=JSPCommand.ADD && iJSPCommand!=JSPCommand.EDIT && iJSPCommand!=JSPCommand.ASK && iErrCode==0 && masterPrivUpdate){%>
                                        <tr align="left" valign="top"> 
                                  ...
                                  <tr align="left" valign="top"> 
                                    <td height="8" valign="middle" colspan="3"> 
                                      <%if((iJSPCommand ==JSPCommand.ADD)||(iJSPCommand==JSPCommand.SAVE)&&(jspPeriodeRekap.errorSize()>0)||(iJSPCommand==JSPCommand.EDIT)||(iJSPCommand==JSPCommand.ASK)){%>
                                      <table width="100%" border="0" cellspacing="1" cellpadding="0">
                                        ...
                                        <tr align="left"> 
                                          <td height="21" valign="middle" width="11%">&nbsp;&nbsp;Nama</td>
                                          <td height="21" colspan="2" width="89%"> 
                                            <input type="text" name="<%=jspPeriodeRekap.colNames[JspPeriodeRekap.JSP_NAME] %>"  value="<%= periode.getName() %>" class="formElemen">
                                            * <%= jspPeriodeRekap.getErrorMsg(JspPeriodeRekap.JSP_NAME) %> 
                                        <tr align="left"> 
                                          <td height="21" valign="middle" width="11%">&nbsp;&nbsp;Mulai 
                                            Tanggal</td>
                                          <td height="21" colspan="2" width="89%"> 
                                            <input type="text" name="<%=jspPeriodeRekap.colNames[JspPeriodeRekap.JSP_START_DATE] %>"  value="<%= JSPFormater.formatDate(periode.getStartDate(),"dd/MM/yyyy") %>" class="formElemen">
                                            * <%= jspPeriodeRekap.getErrorMsg(JspPeriodeRekap.JSP_START_DATE) %> (dd/MM/yyyy) 
                                        <tr align="left"> 
                                          <td height="21" valign="middle" width="11%">&nbsp;&nbsp;Berakhir 
                                            Tanggal </td>
                                          <td height="21" colspan="2" width="89%"> 
                                            <input type="text" name="<%=jspPeriodeRekap.colNames[JspPeriodeRekap.JSP_END_DATE] %>"  value="<%= JSPFormater.formatDate(periode.getEndDate(),"dd/MM/yyyy") %>" class="formElemen">
                                            * <%= jspPeriodeRekap.getErrorMsg(JspPeriodeRekap.JSP_END_DATE) %> (dd/MM/yyyy) 
                                        <tr align="left"> 
                                          <td height="21" valign="middle" width="11%">&nbsp;</td>
                                          <td height="21" colspan="2" width="89%">&nbsp; 
                                        <tr align="left"> 
									...
									String scomDel = "javascript:cmdAsk('"+oidPeriodeRekap+"')";
									String sconDelCom = "javascript:cmdConfirmDelete('"+oidPeriodeRekap+"')";
									String scancel = "javascript:cmdEdit('"+oidPeriodeRekap+"')";
