...
if(iJSPCommand==JSPCommand.START){
	Vector pin = DbPinjaman.list(0,0,"jenis_barang=3 and status=0","");
	out.println("size: "+pin.size());
	for(int i=0; i<pin.size(); i++){
		Pinjaman pinjamanx = (Pinjaman)pin.get(i);
		try{
			pinjamanx.setStatus(1);
			DbPinjaman.updateExc(pinjamanx);
		}
		catch(Exception e){
		}
		DbPinjamanDetail.setupDetailPinjamanWOJurnal(pinjamanx);
		out.println("<br>processing pinjaman i = "+i+", "+pinjamanx.getOID());
	}
}
...
function cmdStartIt(){
	...
	document.frmpinjaman.action="a-start-data-pinjaman-baru.jsp";
