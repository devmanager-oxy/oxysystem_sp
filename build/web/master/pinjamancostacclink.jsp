...
<%!
	...
	{	
		...
		for(int i=0; i<I_Project.pinjamanCostAccLink.length; i++)
		{
			type_key.add(I_Project.pinjamanCostAccLink[i]);
			type_value.add(I_Project.pinjamanCostAccLink[i]);										
		}
...
<%
	...
	if (grpType.equals("All")){
		...
		for(int i=0; i<I_Project.pinjamanCostAccLink.length; i++){
			whereClause = whereClause + " ("+DbAccLink.colNames[DbAccLink.COL_TYPE] + " = '"+ I_Project.pinjamanCostAccLink[i] + "') or"; 	
		}
...
<script language="JavaScript">
	...
	function cmdResetSaja(){
		if(confirm("Warning !! \nthis action will delete previows setup data, and update to the new setup, \nare you seure ?")){
			...
			document.frmacclink.action="pinjamancostacclink.jsp";
                              ...
                              <td height="8"  colspan="3" valign="top"> 
                                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                  <tr align="left" valign="top"> 
									...
									{
							  %>
                                        <option value="<%=I_Project.pinjamanCostAccLink[i]%>" <%if(I_Project.pinjamanCostAccLink[i].equals(grpType)) {%>selected<%}%>><%=I_Project.pinjamanCostAccLink[i]%></option>
                                        <%
							  		}									
							  %>
                                      </select>
