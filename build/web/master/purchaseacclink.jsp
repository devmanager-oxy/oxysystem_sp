...
%>
<%
boolean purchasePriv = QrUserSession.isHavePriviledge(appSessUser.getUserOID(), AppMenu.M1_MENU_PURCHASE, AppMenu.M2_MENU_PURCHASE_ACCLINK);
boolean purchasePrivView = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MENU_PURCHASE, AppMenu.M2_MENU_PURCHASE_ACCLINK, AppMenu.PRIV_VIEW);
boolean purchasePrivUpdate = appSessUser.isPriviledged(appSessUser.getUserOID(), AppMenu.M1_MENU_PURCHASE, AppMenu.M2_MENU_PURCHASE_ACCLINK, AppMenu.PRIV_UPDATE);

%>
<!-- Jsp Block -->
<%!
		
	public String drawList(int iCommand, JspAccLink frmObject, AccLink objEntity, Vector objectClass,  long accLinkId, long oidLocation, boolean isPostableOnly)
	{	
		...
		/*if (objEntity.getDepartmentId() != 0)
		{
			...
			tempCoa = DbCoa.list(0,0, DbCoa.colNames[DbCoa.COL_DEPARTMENT_ID] + " = " + objEntity.getDepartmentId(), "CODE");	
		} else
		{
			...
			tempCoa = DbCoa.list(0,0, DbCoa.colNames[DbCoa.COL_DEPARTMENT_ID] + " = " + dpx.getOID(), "CODE");	
		}*/
		
		String where = "";//" (location_id="+oidLocation+" or location_id=0)";// and "+DbCoa.colNames[DbCoa.COL_COA_CATEGORY_ID]+"<>0";
		...
		{	
			//if(I_Project.accLinkGroup[i].equals(I_Project.ACC_LINK_GROUP_CURRENT_LIABILITIES)){
		...
		for (int i = 0; i < objectClass.size(); i++) 
		{
			...
			if(index == i && (iCommand == JSPCommand.EDIT || iCommand == JSPCommand.ASK || (iCommand==JSPCommand.SAVE && frmObject.errorSize() > 0 )))
			{
				...
				//rowx.add("<div align=\"center\">"+JSPCombo.draw(frmObject.colNames[frmObject.JSP_DEPARTMENT_ID],null, ""+String.valueOf(objEntity.getDepartmentId()).trim(), dept_value , dept_key,  "onChange=\"javascript:cmdDept()\"", "formElemen")+"</div>");
                ...
                <td width="100%" valign="top"> 
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr> 
					  ...
					  <%
					  String navigator = "<font class=\"lvl1\">Purchase</font><font class=\"tit1\">&nbsp;&raquo;&nbsp;<span class=\"lvl2\">Account Link</span></font>";
					  %>
                                  ...
                                  <tr align="left" valign="top"> 
                                    ...
                                    <td height="22" valign="middle" colspan="3" class="comment" width="99%">&nbsp;Account 
                                      ...
                                      <select name="groupType" onChange="javascript:cmdAccGroup()">
                                        <option value="" <%if(grpType.length()==0){%>selected<%}%>>All</option>
                                        <%for(int i=0; i<I_Project.accLinkGroup1.length; i++){%>
...
//								{
						%>
                                  <tr align="left" valign="top"> 
                                    ...
                                    <td height="22" valign="middle" colspan="3" width="99%"><%= drawList(iCommand, jspAccLink, accLink, listAccLink, oidAccLink, sysLocation.getOID(), isPostableOnly)%> 
