...
public class CONHandler implements I_CONType {
    ...
    System.getProperty("file.separator") + "sipadu.xml";    
    ...
    private static final boolean ACTIVATE_LOG = true; //activate logging session    
    ...
    public static final int CONSVR_TYPE  = CONSVR_MYSQL;//CONSVR_POSTGRESQL;  // this is a CON Handler for MySQL
    ...
    protected static int CONMaxConn 	= 100;
    
    public static boolean configLoaded = false;
    public static CONConfigReader cnfReader;// = new CONConfigReader(CONFIG_FILE);
    ...
    //production
    protected static void loadConfig_on() 
    throws CONException {
        if(!configLoaded) {
            getLog();
            try {
                
                CONDriver = "com.mysql.jdbc.Driver";//configReader.getConfigValue("dbdriver");
                CONUrl = "jdbc:mysql://localhost/sipadu";//?user=obisinc&password=h7QFBMeM";//configReader.getConfigValue("dburl");
                CONUser = "root";//configReader.getConfigValue("dbuser");
                CONPwd = "komang_wk";//configReader.getConfigValue("dbpasswd");
                CONMinConn = 5;
                CONMaxConn = 100;

                configLoaded = true;
                System.out.println("/*********************************************************/");
                System.out.println("                        LOAD CONCONFIG             ");
                System.out.println("");
                System.out.println("\t CONDriver : " + CONDriver );
                System.out.println("\t CONUrl    : " + CONUrl);
                System.out.println("\t CONUser   : " + CONUser);
    ...
    throws CONException {
        if(!configLoaded) {
            ...
            try {
                ...
                /*
                CONDriver = "org.gjt.mm.mysql.Driver";//configReader.getConfigValue("CONdriver");
                CONUrl = "jCONc:mysql://10.0.0.54/obisinc_3";//?user=obisinc&password=h7QFBMeM";//configReader.getConfigValue("CONurl");
                CONUser = "obisinc_3";//configReader.getConfigValue("CONuser");
                CONPwd = "danaco";//configReader.getConfigValue("CONpasswd");
                 */
                
                //sspn
                CONDriver = "org.gjt.mm.mysql.Driver";//configReader.getConfigValue("CONdriver");
                CONUrl = "jCONc:mysql://mysql5x.webappcabaret.net/obisinc_6";//?user=obisinc&password=h7QFBMeM";//configReader.getConfigValue("CONurl");
                CONUser = "obisinc_6";//configReader.getConfigValue("CONuser");
                CONPwd = "ssp123";//configReader.getConfigValue("CONpasswd");
                CONMinConn = 5;
                CONMaxConn = 100;
    ...
    throws CONException {
        ...
        try {           
            ...
            if(isSystemLicenseActive()){
                connection = getConnection();
                checkConcurrency(connection); 
                statement = getStatement(connection);
                //CONLog.info("Execute updateSQL: " + getDeleteSQL());
                //System.out.println("delete sql : " + getDeleteSQL());
                statement.executeUpdate(getDeleteSQL());

                if(ACTIVATE_LOG){
                    //CONLog.insertLogs(getDeleteSQL(), tableName, 0);
                }
            }
            else{
                System.out.println(" ");
                System.out.println(" ");
                System.out.println("*****************  ERROR TO DELETE DATA *******************");
                System.out.println("*****************  SYSTEM LICENSE EXPIRED AT "+SYSTEM_DATE_LIMIT+" *******************");
            ...
            switch(getFieldType(i)) {
                ...
                case I_CONType.TYPE_DATE: // '\003'
                    if(s.toUpperCase().equals("CURRENT_TIME")) {
                        ...
                        //System.out.println("CONSVR_TYPE CONSVR_TYPE"+CONSVR_TYPE);
                        switch(CONSVR_TYPE){
	                       	case CONSVR_ORACLE :
    ...
    public int insert() throws CONException {
        ...
        try {
            ...
            if(isSystemLicenseActive()){
                ...
                //CONLog.info("Execute updateSQL: " + getInsertSQL());

                statement.executeUpdate(getInsertSQL());

                if(ACTIVATE_LOG){
                    //CONLog.insertLogs(getInsertSQL(), tableName, 0);
                }
                ...
                System.out.println(" ");
                System.out.println(" ");
                System.out.println("*****************  ERROR TO INSERT DATA *******************");
                System.out.println("*****************  SYSTEM LICENSE EXPIRED AT "+SYSTEM_DATE_LIMIT+" *******************");
                System.out.println("*****************  PLEASE CONTACT YOUR ADMINISTRATOR *******************");
                System.out.println(" ");
                System.out.println(" ");
    ...
    throws CONException {
        ...
        //System.out.println("-- CONHandler : "+doubleVal);
    ...
    throws CONException {
        ...
        try {
            ...
            if(isSystemLicenseActive()){
                ...
                //System.out.println("update sql : " + getUpdateSQL());
                //CONLog.info("Execute updateSQL: " + getUpdateSQL());
                statement.executeUpdate(getUpdateSQL());

                if(ACTIVATE_LOG){
                    //CONLog.insertLogs(getUpdateSQL(), tableName, 0);
                }
            }
            else{
                System.out.println(" ");
                System.out.println(" ");
                System.out.println("*****************  ERROR TO UPDATE DATA *******************");
                System.out.println("*****************  SYSTEM LICENSE EXPIRED AT "+SYSTEM_DATE_LIMIT+" *******************");
                System.out.println("*****************  PLEASE CONTACT YOUR ADMINISTRATOR *******************");
                System.out.println(" ");
                System.out.println(" ");
