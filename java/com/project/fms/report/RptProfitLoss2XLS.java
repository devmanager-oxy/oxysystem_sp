...
public class RptProfitLoss2XLS extends HttpServlet {
    ...
    */
    ...
    throws ServletException, java.io.IOException {
        ...
        try{
            ...
            vectorList = (Vector)session.getValue("PROFIT2");
	...
	if(vDep!=null && vDep.size()>0){
        ...
        wb.println("     <Row>");
        wb.println("      <Cell ss:Index=\"2\" ss:MergeDown=\"2\" ss:StyleID=\"s41\"><Data ss:Type=\"String\">Description</Data></Cell>");
        if(vDep!=null && vDep.size()>0){
            for(int i=0; i<vDep.size(); i++){
                Department dep = (Department)vDep.get(i);												
                Vector vSec = DbDepartment.list(0,0,"level=1 and ref_id="+dep.getOID(),"code");
                if(vSec!=null && vSec.size()>0){															
                    ...
                    wb.println("      <Cell ss:MergeAcross=\""+(vSec.size()+looping==0?0:vSec.size()+looping-1)+"\" ss:StyleID=\"s41\"><Data ss:Type=\"String\">"+dep.getName()+"</Data></Cell>");
                }else{
        ...
        wb.println("      <Cell ss:MergeDown=\"2\" ss:StyleID=\"s41\"><Data ss:Type=\"String\">Total Corporate</Data></Cell>");
        wb.println("     </Row>");
        wb.println("     <Row>");
        int firstSecCol = 0;
        if(vDep!=null && vDep.size()>0){
            for(int i=0; i<vDep.size(); i++){
                ...
                if(vSec!=null && vSec.size()>0){															
                    ...
                    for(int ia=0; ia<vSec.size(); ia++){
                        ...
                        if(firstSecCol==0)
                            wb.println("      <Cell ss:MergeAcross=\""+(vSubSec.size()==0?0:vSubSec.size()-1)+"\" ss:Index=\"3\" ss:StyleID=\"s41\"><Data ss:Type=\"String\">"+depSec.getName()+"</Data></Cell>");
                        else
                            wb.println("      <Cell ss:MergeAcross=\""+(vSubSec.size()==0?0:vSubSec.size()-1)+"\" ss:StyleID=\"s41\"><Data ss:Type=\"String\">"+depSec.getName()+"</Data></Cell>");
                    ...
                    for(int ia=0; ia<vSec.size(); ia++){
                        ...
                        if(vSubSec!=null && vSubSec.size()>0){															
                            for(int ib=0; ib<vSubSec.size(); ib++){
                                ...
                                if(firstSecCol==0)
                                    wb.println("      <Cell ss:Index=\"3\" ss:StyleID=\"s41\"><Data ss:Type=\"String\">"+depSubSec.getName()+"</Data></Cell>");
                                else
                                    wb.println("      <Cell ss:StyleID=\"s41\"><Data ss:Type=\"String\">"+depSubSec.getName()+"</Data></Cell>");
                                firstSecCol = firstSecCol+1;
                            }
                        }else if(firstSecCol==0){
                            wb.println("      <Cell ss:Index=\"3\" ss:StyleID=\"s41\"><Data ss:Type=\"String\"></Data></Cell>");
