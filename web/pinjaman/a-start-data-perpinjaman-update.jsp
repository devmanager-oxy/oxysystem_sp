...
/*************************************************************************
summarry :
untuk melakukan update pinjaman, pinjaman detail dan payment tanpa posting jurnal
khusus untuk kesalahan pinjaman pada % bunga dan jumlah angsuran yang sudah terbayar.
**************************************************************************/ 
%>
<%@ page language = "java" %>
...
String noPinjaman = JSPRequestValue.requestString(request, "pinjaman_num");
long oidPinjamanDetail = JSPRequestValue.requestLong(request, "hidden_pinjaman_detail_id");
double bungaBaru = JSPRequestValue.requestDouble(request, "bunga_baru");
int angsurTrakhir = JSPRequestValue.requestInt(request, "angsur_terakhir");
double totalPinjaman = JSPRequestValue.requestDouble(request, "total_pinjaman");
int jmlAngsuran = JSPRequestValue.requestInt(request, "jml_angsuran");
String txtDate = JSPRequestValue.requestString(request, "txt_date");

Pinjaman pinjaman = new Pinjaman();

if(noPinjaman.length()>0 && iJSPCommand==JSPCommand.START){

	Vector v = DbPinjaman.list(0,0, "number='"+noPinjaman+"'", "");
	if(v!=null && v.size()>0){
		
		try{			
			pinjaman = (Pinjaman)v.get(0);
			pinjaman = DbPinjaman.fetchExc(pinjaman.getOID());
			pinjaman.setBunga(bungaBaru);
			pinjaman.setAngsuranTerakhir(angsurTrakhir);
			pinjaman.setTotalPinjaman(totalPinjaman);
			pinjaman.setLamaCicilan(jmlAngsuran);
			pinjaman.setDate(JSPFormater.formatDate(txtDate, "dd/MM/yyyy"));
			
			DbPinjaman.updateExc(pinjaman);
			
			//Pinjaman pinjamanx = DbPinjaman.fetchExc(pinjaman.getOID());
			DbPinjamanDetail.setupDetailPinjamanWOJurnal(pinjaman);
			out.println("<br>processing pinjaman :"+pinjaman.getOID());
		}
		catch(Exception e){
		}
...
function cmdStartIt(){
	...
	document.frmpinjaman.action="a-start-data-perpinjaman-update.jsp";
	document.frmpinjaman.submit();
}

function cmdUpdateTanggal(oid){
	document.frmpinjaman.pinjaman_num.value="0";
...
function cmdUpdateJT(){
	document.frmpinjaman.pinjaman_num.value="<%=pinjaman.getOID()%>";
...
function cmdAsk(oidPinjaman){
	document.frmpinjaman.pinjaman_num.value=oidPinjaman;
                        ...
                        <form name="frmpinjaman" method ="post" action="">
                          ...
                          <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr align="left" valign="top"> 
                              <td height="8"  colspan="3">&nbsp;&nbsp;pinjaman number 
                                <input type="text" name="pinjaman_num">
                                , approved date 
                                <input type="text" name="txt_date" value="dd/mm/yyyy">
                                , total pinjaman baru 
                                <input type="text" name="total_pinjaman">
                                ,<br>
                                jml angsuran 
                                <input type="text" name="jml_angsuran" size="10">
                                , bunga baru 
                                <input type="text" name="bunga_baru" size="10">
                                , angsuran terakhir 
                                <input type="text" name="angsur_terakhir" size="10">
                              </td>
                            </tr>
                            <tr align="left" valign="top"> 
